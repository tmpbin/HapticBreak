...
### Changed
- **后台空闲精修**：菜单栏刷新与视图模型同步改为「值变了才写」——暂停或 iconOnly/progressRing
  样式下菜单栏不再每秒空重绘，progressRing 图标按进度量化缓存；手动暂停期间跳过全屏/麦克风/
  专注模式环境探测（恢复后立即重探）。后台稳定在 ~0.8%。

### Fixed
- **修复统计窗口重开后显示旧数据**：窗口关闭仅隐藏（复用实例），SwiftUI `onAppear` 只在首次打开
  触发，统计快照因此冻结在第一次打开的时刻。现每次呈现统计窗口都会广播刷新，重开即见最新数据。
- **修复 `--rendershots` 可能覆盖真实用户设置**：该调试入口会把 `Settings.shared` 改写为「新装机
  代表值」用于截图；现将其纳入临时运行模式（内存态存储，进程退出即弃），对打包后的 .app 执行也
  不会再污染用户偏好。
- **修复面板关闭后的后台高 CPU 占用**：圆环流光此前以命令式 `repeatForever` 驱动共享的
  `RingAnimator`（`@Published`），而「禁用动画的事务」无法可靠取消 `repeatForever`；叠加弹窗关闭后
  宿主视图仍被保留，导致整只圆环在后台被逐帧重绘（无窗口可见时空闲占用高达 ~40%）。现将流光改由其
  自身视图的局部 `@State` 驱动（视图离开视图树时 SwiftUI 会自动取消其动画），并在面板收起时把内容
  折叠为惰性占位。无前台窗口时占用降至 <1%。
- **修复面板打开时的前台高 CPU 占用（~10% → ~3%）**：圆环的两处持续动画（流光旋转、秒针扫动）
  此前由 SwiftUI 驱动——`repeatForever` 与每秒接力的 `withAnimation` 会让整个面板的视图图在
  120Hz ProMotion 下逐帧于 app 进程内重求值。现改为 Core Animation 层实现
  （`ShimmerOrnament` / `SecondHandOrnament`）：app 每秒只提交 O(1) 个事务，逐帧插值由渲染服务器
  进程外完成；每秒尖端脉冲、每分钟后坐力与冲击波同样为一次性 CA 动画，视觉观感不变。
  完整圆环从 6.2% 降至无动画基线 1.6%，整面板从 9.3% 降至 ~3%。归因与实验记录见
  `docs/PANEL_CPU_INVESTIGATION.md`。
